...
function ENT.bouncePlayer( trampoline, ply, plyPhys, speed )
    ...
    if isUnfrozen then
        ...
        -- NOTE: This breaks prediction + Is this really needed?
        plyPhys:SetPos( plyPhys:GetPos() + up * 5 )
    end

    local appliedVelocity = up * bounceSpeed
    print( appliedVelocity, upSpeed, bounceSpeed, trampoline:GetAngles() )
    ply:SetVelocity( appliedVelocity )
    if speed > 0 then
    	--print(appliedVelocity)
    end
...
hook.Add( "SetupMove", "Trampoline_Prediction", function( ply, mv, cmd ) -- BUG: If the Entity is Angled, there will be one prediction error for some reason :<
	...
	if ent != NULL then
		...
		if SERVER and ply:GetMoveType() != MOVETYPE_NOCLIP then
			print( LAST_VELOCITY[ plyIndex ], FrameTime() )
		end
	end
end )